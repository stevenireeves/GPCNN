...
def get_batch(train_in, train_gt, batch_size, shift, m):
  block = m*batch_size - shift*batch_size if (shift+1)*batch_size > m*batch_size else (shift+1)*batch_size
  train_x = torch.from_numpy(train_in[shift*batch_size:block])
  train_y = torch.from_numpy(train_gt[shift*batch_size:block])
